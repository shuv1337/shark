...
- Production secrets use separate `SHark Production App` and `SHark Production Backup` 1Password vaults. The VM fetches them through separate read-only service accounts; neither runtime account can access the other vault.
...
- The Restic password exists only in the isolated 1Password backup vault; repository verification is a quarterly restore drill with no additional weekly or monthly check job.
